...
export default function ServiceDetail() {
  ...
  const [selectedTierIds, setSelectedTierIds] = useState<string[]>([]); // ✅ Array untuk multi-select
  ...
  async function fetchServiceData() {
    ...
    if (serviceData) {
      ...
      // ✅ Hitung total jumlah review (untuk link "View All")
      ...
      // ✅ Hanya ambil 3 review terbaru untuk preview
  ...
  // ✅ Hitung total harga dari semua package terpilih
  ...
  async function handleAddToCart(e: FormEvent<HTMLFormElement>) {
    ...
    addToCart({
      ...
      tierNames: getSelectedTierNames(),
    ...
    await showAlert({
      title: 'Added to Cart',
      message: `${service!.name} (${getSelectedTierNames().join(', ')}) has been added to your cart.`,
      type: 'success'
    });
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="glass-card rounded-2xl p-6">
              ...
              {/* Rating Summary (Klik untuk ke halaman semua ulasan) */}
              <Link
              ...
              >
                <div className="flex items-center gap-1">
                  {[1, 2, 3, 4, 5].map((star) => (
                    <svg
                      key={star}
                      className={`w-5 h-5 ${star <= Math.round(averageRating) ? 'text-yellow-400' : 'text-zinc-600'}`}
                      fill="currentColor"
                      viewBox="0 0 20 20"
                    >
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div
                              className={`w-5 h-5 rounded border-2 flex items-center justify-center flex-shrink-0 transition-colors ${
                                isSelected ? 'bg-primary border-primary' : 'border-zinc-600 bg-zinc-900'
                              }`}
                            >
                              {isSelected && (
                            ...
                            <div>
                              ...
                              <p className="text-sm text-zinc-400">
                                {tier.description || `Est. ${tier.estimated_hours || service.estimated_hours} hours`}
                              </p>
                            </div>
                          </div>
                      ...
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <div className="flex gap-0.5">
                          {[1, 2, 3, 4, 5].map((star) => (
                            <svg
                              key={star}
                              className={`w-4 h-4 ${star <= (review.rating || 0) ? 'text-yellow-400' : 'text-zinc-600'}`}
                              fill="currentColor"
                              viewBox="0 0 20 20"
                            >
                              <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
              ...
              <div className="mb-6 p-4 bg-zinc-900/50 rounded-xl border border-border space-y-2">
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span className="text-white font-medium text-right">
                    {selectedTierIds.length > 0 ? getSelectedTierNames().join(', ') : 'None selected'}
                  </span>
                </div>
                <div className="flex justify-between text-sm">
              ...
              <form onSubmit={handleAddToCart} className="space-y-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    required
                    className="input-modern"
                    placeholder="e.g., Asia, NA, Server 1"
                  />
                </div>
