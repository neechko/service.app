...
export default function AdminDashboard() {
  ...
  async function fetchAllData() {
    ...
    const [ordersRes, workersRes, servicesRes, categoriesRes, usersRes, packagesRes] = await Promise.all([
      ...
      supabase.from("service_tiers").select("*, services(name)").order("created_at", { ascending: false }),
    ]);
                  ...
                  <tbody>
                    {packages.map((pkg) => (
                      <tr key={pkg.id} className="border-b border-border hover:bg-surface/30 transition">
                        ...
                        <td className="px-6 py-4 text-zinc-300">{formatRupiah(pkg.fixed_price || 0)}</td>
                        <td className="px-6 py-4 text-zinc-300">{pkg.estimated_hours || 1}h</td>
                        <td className="px-6 py-4">
              ...
              <form onSubmit={handleSaveService} className="space-y-4">
                ...
                <div>
                  ...
                  <select value={serviceForm.category} onChange={(e: ChangeEvent<HTMLSelectElement>) => setServiceForm({ ...serviceForm, category: e.target.value })} required className="input-modern">
                    ...
                    {categories.filter((c) => c.is_active).map((cat) => (<option key={cat.id} value={cat.slug}>{cat.name}</option>))}
                  </select>
                </div>
                <div>
            ...
            <div className="glass-card rounded-2xl p-6 w-full max-w-md">
              ...
              <form onSubmit={handleSavePackage} className="space-y-4">
                ...
                <div>
                  ...
                  <input type="text" value={packageForm.name} onChange={(e: ChangeEvent<HTMLInputElement>) => setPackageForm({ ...packageForm, name: e.target.value })} required placeholder="e.g., Floor 9, Floor 12" className="input-modern w-full" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-zinc-300 mb-1">Description (Optional)</label>
                  <textarea value={packageForm.description} onChange={(e: ChangeEvent<HTMLTextAreaElement>) => setPackageForm({ ...packageForm, description: e.target.value })} rows={2} placeholder="e.g., Complete all chambers" className="input-modern w-full resize-none" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-zinc-300 mb-1">Fixed Price (IDR)</label>
                  <input type="number" min="0" value={packageForm.fixed_price || 0} onChange={(e: ChangeEvent<HTMLInputElement>) => setPackageForm({ ...packageForm, fixed_price: parseInt(e.target.value) || 0 })} required className="input-modern w-full" />
                  <p className="text-xs text-zinc-500 mt-1">Set the exact price for this package (e.g., 10000 for Rp 10,000)</p>
                </div>
                <div>
                  <label className="block text-sm font-medium text-zinc-300 mb-1">Estimated Hours</label>
                  <input type="number" min="1" value={packageForm.estimated_hours || 1} onChange={(e: ChangeEvent<HTMLInputElement>) => setPackageForm({ ...packageForm, estimated_hours: parseInt(e.target.value) || 1 })} required className="input-modern w-full" />
                </div>
                <div className="flex items-center gap-2">
