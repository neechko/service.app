...
// Tipe Form (lebih sederhana, hanya field yang bisa diedit)
type ServiceForm = {
  name: string;
  category: string;
  description: string;
...
export default function AdminDashboard() {
  ...
  async function fetchAllData() {
    setLoading(true);
    const [ordersRes, workersRes, servicesRes, categoriesRes] =
      await Promise.all([
        supabase
          .from("orders")
  ...
  // ✅ DIPERBAIKI: Menerima null/undefined agar aman dipanggil dari mana saja
              ...
              orders.map((order) => (
                <div key={order.id} className="glass-card rounded-2xl p-6">
                  <div className="flex justify-between items-start mb-4">
                    ...
                    <span
                      className={`px-3 py-1 rounded-md text-xs font-semibold uppercase border ${
                  ...
                  <div className="flex gap-2">
                    ...
                    {!order.worker_id && (
                      <button
                        ...
                        className="flex-1 bg-primary hover:bg-primary-hover text-white font-semibold py-2 rounded-lg text-sm transition-all"
                      >
                        Assign Worker
                      </button>
                    )}
                  ...
                  >
                    ...
                    {workers.map((worker) => (
                      <option key={worker.id} value={worker.id}>
                        {worker.full_name} ({worker.phone || "No phone"})
                      </option>
                    ))}
                  </select>
