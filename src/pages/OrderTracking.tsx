  ...
  async function handleProgressUpdate(e: FormEvent<HTMLFormElement>) {
    ...
    if (!progressFile) {
      await showAlert({
        ...
        message: "Screenshot bukti progres wajib diupload!",
        type: "warning",
      });
      return;
    }
    ...
    try {
      ...
      await showAlert({
        title: "Error",
        message: "Gagal update progres: " + (err as Error).message,
        type: "danger",
      });
    }
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="glass-card rounded-2xl p-6 mb-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
            ...
            <span
              className={`px-3 py-1 rounded-md text-xs font-semibold uppercase border w-fit ${getStatusStyle(order.status)}`}
            >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4 border-t border-border">
            <div>
              ...
              <p className="text-white font-medium">
                {order.consumer_profile?.full_name || "-"}
              </p>
            ...
            <div>
              ...
              <p className="text-white font-medium">
                {order.worker_profile?.full_name || "Unassigned"}
              </p>
              <p className="text-zinc-400 text-sm">
                {order.worker_profile?.phone || "-"}
              </p>
            </div>
            <div>
          ...
          {order.status === "cancelled" && (
            <div className="mt-4 p-4 bg-red-500/10 border border-red-500/30 rounded-lg">
              ...
              {order.cancel_reason && (
                <p className="text-red-300 text-sm italic mb-2 ml-7">
                  Reason: "{order.cancel_reason}"
                </p>
              )}
              {order.cancelled_at && (
                <p className="text-red-400/70 text-xs ml-7">
          ...
          {progressUpdates.length === 0 ? (
            <div className="text-center py-8">
              ...
              <p className="text-zinc-600 text-sm mt-1">
                The worker will upload progress here.
              </p>
            </div>
          ) : (
                ...
                <div className="bg-surface/50 rounded-xl p-4 border border-border">
                  <div className="flex items-center gap-1 mb-2">
                    {[1, 2, 3, 4, 5].map((star) => (
                      <svg
                        key={star}
                        className={`w-5 h-5 ${star <= (existingReview.rating || 0) ? "text-yellow-400" : "text-zinc-600"}`}
                        fill="currentColor"
                        viewBox="0 0 20 20"
                      >
                        <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                ...
                <form onSubmit={handleSubmitReview} className="space-y-4">
                  <div>
                    ...
                    <div className="flex gap-2">
                      {[1, 2, 3, 4, 5].map((star) => (
                        ...
                        >
                          <svg
                            className={`w-8 h-8 ${star <= newRating ? "text-yellow-400" : "text-zinc-600"}`}
                            fill="currentColor"
                            viewBox="0 0 20 20"
                          >
                            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      placeholder="Bagaimana pengalaman Anda dengan worker ini?"
                    />
                  </div>
                  <button
                    type="submit"
                    disabled={submittingReview}
                    className="bg-primary hover:bg-primary-hover disabled:bg-zinc-800 text-white font-semibold py-2.5 px-6 rounded-lg transition-all"
                  >
